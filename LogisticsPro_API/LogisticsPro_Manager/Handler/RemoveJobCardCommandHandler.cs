...
{
    public class RemoveJobCardCommandHandler : IRequestHandler<RemoveJobCardCommand, RequestRemoveEnvelop>
    {
        private readonly IJobFactory jobFactory;

        public RemoveJobCardCommandHandler(DB_LogisticsproContext dB_LogisticsproContext)
        {
            this.jobFactory = new JobFactory(dB_LogisticsproContext);
        }

        public async Task<RequestRemoveEnvelop> Handle(RemoveJobCardCommand removeJobCardCommand,CancellationToken cancellationToken)
        {
            var jobDomain = this.jobFactory.Create();
            try
            {
                var response = await jobDomain.RemoveJobCard(removeJobCardCommand.Id);
