...
{
    public class RemoveVendorCommandHandler : IRequestHandler<RemoveVendorCommand, RequestRemoveEnvelop>
    {
        private readonly IMasterFactory masterFactory;

        public RemoveVendorCommandHandler(DB_LogisticsproContext dB_LogisticsproContext)
        {
            this.masterFactory = new MasterFactory(dB_LogisticsproContext);
        }

        public async Task<RequestRemoveEnvelop> Handle(RemoveVendorCommand removeVendorCommand,CancellationToken cancellationToken)
        {
            var masterDomain = this.masterFactory.Create();
            try
            {
                var response = await masterDomain.RemoveVendor(removeVendorCommand.Id);
