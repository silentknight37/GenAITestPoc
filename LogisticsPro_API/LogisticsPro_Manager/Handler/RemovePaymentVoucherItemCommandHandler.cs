...
{
    public class RemovePaymentVoucherItemCommandHandler : IRequestHandler<RemovePaymentVoucherItemCommand, RequestRemoveEnvelop>
    {
        private readonly ILogisticsFactory logisticsFactory;

        public RemovePaymentVoucherItemCommandHandler(DB_LogisticsproContext dB_LogisticsproContext)
        {
            this.logisticsFactory = new LogisticsFactory(dB_LogisticsproContext);
        }

        public async Task<RequestRemoveEnvelop> Handle(RemovePaymentVoucherItemCommand removePaymentVoucherItemCommand,CancellationToken cancellationToken)
        {
            var logisticsDomain = this.logisticsFactory.Create();
            try
            {
                var response = await logisticsDomain.RemovePaymentVoucherItemFromList(removePaymentVoucherItemCommand.Id, removePaymentVoucherItemCommand.ItemType);
