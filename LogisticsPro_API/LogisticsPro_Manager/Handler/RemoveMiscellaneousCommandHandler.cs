...
{
    public class RemoveMiscellaneousCommandHandler : IRequestHandler<RemoveMiscellaneousCommand, RequestRemoveEnvelop>
    {
        private readonly IJobFactory jobFactory;

        public RemoveMiscellaneousCommandHandler(DB_LogisticsproContext dB_LogisticsproContext)
        {
            this.jobFactory = new JobFactory(dB_LogisticsproContext);
        }

        public async Task<RequestRemoveEnvelop> Handle(RemoveMiscellaneousCommand removeMiscellaneousCommand,CancellationToken cancellationToken)
        {
            var jobDomain = this.jobFactory.Create();
            try
            {
                var response = await jobDomain.RemoveMiscellaneous(removeMiscellaneousCommand.Id);
