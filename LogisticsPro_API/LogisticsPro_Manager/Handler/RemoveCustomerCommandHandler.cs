...
{
    public class RemoveCustomerCommandHandler : IRequestHandler<RemoveCustomerCommand, RequestRemoveEnvelop>
    {
        private readonly IMasterFactory masterFactory;

        public RemoveCustomerCommandHandler(DB_LogisticsproContext dB_LogisticsproContext)
        {
            this.masterFactory = new MasterFactory(dB_LogisticsproContext);
        }

        public async Task<RequestRemoveEnvelop> Handle(RemoveCustomerCommand removeCustomerCommand,CancellationToken cancellationToken)
        {
            var masterDomain = this.masterFactory.Create();
            try
            {
                var response = await masterDomain.RemoveCustomer(removeCustomerCommand.Id);
