...
{
    public class RemoveProformaInvoiceReceiptCommandHandler : IRequestHandler<RemoveProformaInvoiceReceiptCommand, RequestRemoveEnvelop>
    {
        private readonly ILogisticsFactory logisticsFactory;

        public RemoveProformaInvoiceReceiptCommandHandler(DB_LogisticsproContext dB_LogisticsproContext)
        {
            this.logisticsFactory = new LogisticsFactory(dB_LogisticsproContext);
        }

        public async Task<RequestRemoveEnvelop> Handle(RemoveProformaInvoiceReceiptCommand removeProformaInvoiceReceiptCommand,CancellationToken cancellationToken)
        {
            var logisticsDomain = this.logisticsFactory.Create();
            try
            {
                var response = await logisticsDomain.RemoveProformaInvoiceReceipt(removeProformaInvoiceReceiptCommand.Id);
