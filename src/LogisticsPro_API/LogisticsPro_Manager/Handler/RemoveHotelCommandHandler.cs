...
{
    public class RemoveHotelCommandHandler : IRequestHandler<RemoveHotelCommand, RequestRemoveEnvelop>
    {
        private readonly IJobFactory jobFactory;

        public RemoveHotelCommandHandler(DB_LogisticsproContext dB_LogisticsproContext)
        {
            this.jobFactory = new JobFactory(dB_LogisticsproContext);
        }

        public async Task<RequestRemoveEnvelop> Handle(RemoveHotelCommand removeHotelCommand,CancellationToken cancellationToken)
        {
            var jobDomain = this.jobFactory.Create();
            try
            {
                var response = await jobDomain.RemoveHotel(removeHotelCommand.Id);
