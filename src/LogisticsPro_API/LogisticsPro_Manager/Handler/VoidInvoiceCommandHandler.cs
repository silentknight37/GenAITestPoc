...
{
    public class VoidInvoiceCommandHandler : IRequestHandler<VoidInvoiceCommand, RequestRemoveEnvelop>
    {
        private readonly ILogisticsFactory logisticsFactory;

        public VoidInvoiceCommandHandler(DB_LogisticsproContext dB_LogisticsproContext)
        {
            this.logisticsFactory = new LogisticsFactory(dB_LogisticsproContext);
        }

        public async Task<RequestRemoveEnvelop> Handle(VoidInvoiceCommand voidInvoiceCommand,CancellationToken cancellationToken)
        {
            var logisticsDomain = this.logisticsFactory.Create();
            try
            {
                var response = await logisticsDomain.VoidInvoice(voidInvoiceCommand.Id, voidInvoiceCommand.UserId);
