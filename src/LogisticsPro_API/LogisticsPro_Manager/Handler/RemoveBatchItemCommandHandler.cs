...
{
    public class RemoveBatchItemCommandHandler : IRequestHandler<RemoveBatchItemCommand, RequestRemoveEnvelop>
    {
        private readonly ILogisticsFactory logisticsFactory;

        public RemoveBatchItemCommandHandler(DB_LogisticsproContext dB_LogisticsproContext)
        {
            this.logisticsFactory = new LogisticsFactory(dB_LogisticsproContext);
        }

        public async Task<RequestRemoveEnvelop> Handle(RemoveBatchItemCommand removeBatchItemCommand,CancellationToken cancellationToken)
        {
            var logisticsDomain = this.logisticsFactory.Create();
            try
            {
                var response = await logisticsDomain.RemoveBatchItemFromList(removeBatchItemCommand.Id);
