...
{
    public class OpenJobCardCommandHandler : IRequestHandler<OpenJobCardCommand, RequestSaveEnvelop>
    {
        private readonly IJobFactory jobFactory;

        public OpenJobCardCommandHandler(DB_LogisticsproContext dB_LogisticsproContext)
        {
            this.jobFactory = new JobFactory(dB_LogisticsproContext);
        }

        public async Task<RequestSaveEnvelop> Handle(OpenJobCardCommand openJobCardCommand,CancellationToken cancellationToken)
        {
            var jobDomain = this.jobFactory.Create();
            try
            {
                var openJobCardRequest = new OpenJobCardRequest
                {  Id= openJobCardCommand.Id,
                   UserId= openJobCardCommand.UserId
                };

                var response = await jobDomain.OpenJobCard(openJobCardRequest);
