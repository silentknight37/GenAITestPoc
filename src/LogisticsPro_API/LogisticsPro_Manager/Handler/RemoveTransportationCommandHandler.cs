...
{
    public class RemoveTransportationCommandHandler : IRequestHandler<RemoveTransportationCommand, RequestRemoveEnvelop>
    {
        private readonly IJobFactory jobFactory;

        public RemoveTransportationCommandHandler(DB_LogisticsproContext dB_LogisticsproContext)
        {
            this.jobFactory = new JobFactory(dB_LogisticsproContext);
        }

        public async Task<RequestRemoveEnvelop> Handle(RemoveTransportationCommand removeTransportationCommand,CancellationToken cancellationToken)
        {
            var jobDomain = this.jobFactory.Create();
            try
            {
                var response = await jobDomain.RemoveTransportation(removeTransportationCommand.Id);
